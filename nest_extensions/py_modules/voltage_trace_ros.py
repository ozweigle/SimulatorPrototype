...
def init():
	print 'voltage_trace_ros'
	try:
		rospy.init_node('voltage_trace_ros')
	except rospy.exceptions.ROSInitException:
		print >> sys.stderr, 'voltage_trace_ros is unable to initialize node. Master may not be running.'
		sys.exit(0)
...
def from_device(detec, neurons=None, title=None, grayscale=False, timeunit="ms"):
    ...
    voltage_state_pub = rospy.Publisher('/nest/voltage', Float32)
    ...
    if nest.GetStatus(detec, "to_memory")[0]:
        ...
        for neuron in neurons:
            ...
            try:
				for voltage in voltages[neuron]:
					voltage_msg = Float32()
					voltage_msg.data = voltage
					voltage_state_pub.publish(voltage_msg)
					time.sleep(0.05)
